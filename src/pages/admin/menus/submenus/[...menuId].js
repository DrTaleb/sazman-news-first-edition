...
export default function sliders({data}) {
    ...
    function createData(id, title, link, childrenCount , status,subMenus, options) {
        return {id, title,link, childrenCount, status,subMenus, options};
    }
    ...
    useEffect(()=>{
       if(DATA.id){
           DATA.map(item => rows.push(createData(`${item.id}`, `${item.title}`,`${item.link}`, `${item.children.length}`,  `${item.status === 1 ? "فعال" : "غیر فعال"}`),))
...
export async function getServerSideProps(context){
    ...
    const dataResponse = await fetch(`https://newsapi.deltagroup.ir/panel/menus?type=${params.menuId[0]}&parent_id=${params.menuId[1]}&page=1&limit=1000`,{
        method : "GET",
        headers : {
