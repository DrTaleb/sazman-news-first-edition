...
export default function SubMenus({data}) {
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/admin/menus/header/submenus/${router.query.menuId}`)
...
export async function getServerSideProps(context){
    ...
    const dataResponse = await fetch(`${process.env.SERVER_URL}/panel/menus/${params.menuId}?type=header`,{
        method : "GET",
        headers : {
