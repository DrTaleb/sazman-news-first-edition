...
export default function Menus({data}) {
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/admin/menus/header/${router.query.page}`)
                ...
                <div className={"col-5 col-sm-4 col-md-3 col-lg-2"}>
                    <div className={"d-flex flex-row justify-content-center"}>
                        <Link href={"/admin/menus/header/add-menu"} className={"ps-2"}>
                            <Button variant={"contained"} className={"bg-my-purple"}>افزودن منو و زیرمنو</Button>
                        </Link>
                    </div>
                </div>
            </div>
...
export async function getServerSideProps(context) {
    ...
    const dataResponse = await fetch(`${process.env.SERVER_URL}/panel/menus?type=header&page=${params.page}&limit=10`, {
        method: "GET",
        headers: {
