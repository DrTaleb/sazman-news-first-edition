...
export default function SelectedPosts({data}) {
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/admin/post-requests/${router.query.page}?${searchCategory}=${nameSearch}`)
        ...
        <div className={"px-md-4"}>
            ...
            <Paper className={"p-md-3 pt-3 mt-3"}
                ...
                <div className={"d-flex flex-row flex-wrap gap-3 px-3 px-md-0"}>
                    ...
                    <Button variant={"contained"} onClick={search} className={"align-self-center bg-my-purple col-12 col-md-2 col-xl-1 "}>
                        جستجو
                    </Button>
                </div>                <TableContainer sx={{maxHeight: 600}}>
...
export async function getServerSideProps(context) {
    try {
        ...
        const dataResponse = await fetch(`${process.env.SERVER_URL}/panel/posts?page=${params.page}&limit=10&status=0`, {
            method: "GET",
            headers: {
