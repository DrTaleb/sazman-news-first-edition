...
export default function Companies() {
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/user-panel/gallery/${router.query.page}`)
    ...
    const editHandler = (id) => {
        router.push(`/user-panel/gallery/edit/${id}`)
    }
    ...
    const blockHandler = async (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const res = await axios.put(`${process.env.LOCAL_URL}/api/user-panel/gallery/edit/${id}`, formData, {
                            headers: {
    ...
    const deleteHandler = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Nprogress.start()
                try {
                    fetch(`${process.env.LOCAL_URL}/api/user-panel/gallery/delete/${id}`, {
                        method: "DELETE"
    ...
    const clickHandler = async (event, value) => {
        await router.push(`/user-panel/gallery/${value}`)
        await setGeData(!getData)
    }
