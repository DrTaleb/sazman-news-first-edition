...
export default function Companies() {
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/user-panel/certificates/${router.query.page}`)
    ...
    if (DATA.status) {
        DATA.data.data.map(item => rows.push(createData(`${item.id}`, `${item.title}`, `${item.status == 1 ? "فعال" : "غیر فعال"}`)))
    }
    const editHandler = (id) => {
        router.push(`/user-panel/certificates/edit/${id}`)
    }
    ...
    const blockHandler = async (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const res = await axios.put(`${process.env.LOCAL_URL}/api/user-panel/certificates/edit/${id}`, formData, {
                            headers: {
    ...
    const deleteHandler = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Nprogress.start()
                try {
                    fetch(`${process.env.LOCAL_URL}/api/user-panel/certificates/delete/${id}`, {
                        method: "DELETE"
    ...
    const clickHandler = async (event, value) => {
        await router.push(`/user-panel/certificates/${value}`)
        await setGeData(!getData)
    }
