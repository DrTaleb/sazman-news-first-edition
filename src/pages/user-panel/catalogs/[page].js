...
export default function Companies() {
    ...
    const editHandler = (id) => {
        router.push(`/user-panel/catalogs/edit/${id}`)
    }
    ...
    const blockHandler = async (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const res = await axios.put(`${process.env.LOCAL_URL}/api/user-panel/catalogs/edit/${id}`, formData, {
                            headers: {
    ...
    const deleteHandler = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Nprogress.start()
                try {
                    fetch(`${process.env.LOCAL_URL}/api/user-panel/catalogs/delete/${id}`, {
                        method: "DELETE"
    ...
    const clickHandler = async (event, value) => {
        await router.push(`/user-panel/catalogs/${value}`)
        await setGeData(!getData)
    }
                ...
                <div className="d-flex flex-row align-items-center mt-2 mt-md-0">
                    ...
                    <div className={"col-5 col-sm-4 col-md-3 col-lg-2"}>
                        <div className={"d-flex flex-row justify-content-center"}>
                            <Link href={"/user-panel/catalogs/add"}>
                                <Button variant={"contained"} className={"bg-my-purple"}>افزودن کاتالوگ</Button>
                            </Link>
                        </div>
                    </div>
                </div>
